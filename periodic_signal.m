...
global N_period n_point freq origin_signal mix_signal result func_input
...
% Last Modified by GUIDE v2.5 15-May-2012 01:18:34
...
%get_string =strcat('@(x) ',get(hObject,'string'));
%func_input = str2func(get_string);
...
function freqency_Callback(hObject, eventdata, handles)
global N_period n_point freq origin_signal mix_signal result func_input
% hObject    handle to freqency (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of freqency as text
%        str2double(get(hObject,'String')) returns contents of freqency as a double
freq = str2double(get(hObject,'string'));


% --- Executes during object creation, after setting all properties.
function freqency_CreateFcn(hObject, eventdata, handles)
% hObject    handle to freqency (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% handles    structure with handles and user data (see GUIDATA)
h1=findobj('Tag','func');
get_string =strcat('@(x) ',get(h1,'string'));
func_input = str2func(get_string);
...
grid on;
origin_signal = gen_signal(func_input,1/freq,N_period,n_point);
plot([0:1/(freq*n_point):1/freq],origin_signal(1,:))

% --- Executes on button press in mix_noise.
function mix_noise_Callback(hObject, eventdata, handles)
global N_period n_point freq origin_signal mix_signal result amp power_signal
...
%snr = 10*log10(max(origin_signal(1,:))/amp);
s1 =  sprintf('%f', snr);
...
set(gca,'NextPlot','add');
grid on;
p1 = plot([0:1/(freq*n_point):1/freq],mix_signal(1,:));
set(p1,'Color','blue')
p2 = plot([0:1/(freq*n_point):1/freq],origin_signal(1,:));
set(p2,'Color','red')


% --- Executes on button press in detect_signal.
function detect_signal_Callback(hObject, eventdata, handles)
global N_period n_point freq origin_signal mix_signal result func_input power_signal
% hObject    handle to detect_signal (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)


result100 = signal_averaging(mix_signal,N_period/100);
...
set(gca,'NextPlot','add');
grid on;
plot([0:1/(freq*n_point):1/freq],result100(1,:) )


result10 = signal_averaging(mix_signal,N_period/10);
...
set(gca,'NextPlot','add');
grid on;
plot([0:1/(freq*n_point):1/freq],result10(1,:) )



result = signal_averaging(mix_signal,N_period);
...
set(gca,'NextPlot','add');
grid on;
plot([0:1/(freq*n_point):1/freq],result(1,:) )
...
global N_period n_point freq origin_signal mix_signal result amp
